...
const App = () => {
  ...
  const [notes, noteService] = useResource('http://localhost:3005/notes') as [Note[], { create: (note : Omit<Note, "id">) => void }]
  const [persons, personService] = useResource('http://localhost:3005/persons') as [Person[], { create: (person : Omit<Person, "id">) => void }]
